...
//            $content->description('Description...');
            ...
            $content->row(function (Row $row) {
                $row->column(2, new InfoBox('Подача заявлений', 'users', 'aqua', '/cp/statements', '1024'));
                $row->column(2, new InfoBox('Новости', 'newspaper-o', 'yellow', '/cp/news', '2786'));
                $row->column(2, new InfoBox('Страницы', 'book', 'blue', '/cp/pages', '2786'));
                $row->column(2, new InfoBox('Альбомы', 'picture-o', 'green', '/cp/albums', '2786'));
                $row->column(2, new InfoBox('Ответы', 'question-circle', 'gray', '/cp/polls', '150%'));
                $row->column(2, new InfoBox('Документы', 'file', 'red', '/cp/documents', '698726'));
            });
